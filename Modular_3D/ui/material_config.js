(function(){
  ...
  var INCLUDED_TEXTURES=[
    {id:'blanco_liso',name:'Blanco liso',file:'blanco_liso.png',defaultScale:600},
    {id:'blanco_nube',name:'Blanco nube',file:'blanco_nube.png',defaultScale:600},
    {id:'negro_mate',name:'Negro mate',file:'negro_mate.png',defaultScale:600},
    {id:'gris_antracita',name:'Gris antracita',file:'gris_antracita.png',defaultScale:600},
    {id:'roble_claro',name:'Roble claro',file:'roble_claro.png',defaultScale:300},
  ...
  function pieces(){return window.Modular3DView&&window.Modular3DView.getPieces?window.Modular3DView.getPieces():[];}
  ...
  function resolved(piece){var key=piece.materialKey||piece.pieceId,custom=overrides[key],group=custom&&custom.group||groupFor(piece);if(custom&&custom.color)return custom.color;if(checked('material_unico')&&!checked('material_'+group+'_custom'))return value('material_global_color','#ffffff');return groupColor(group);}
  ...
  function selectDetail(detail){
    ...
    if(id('material_piece_oversize_alto'))id('material_piece_oversize_alto').value=sizing.delta_alto||0;
    var miter=miterOverrides[selectedKey]||{};
    if(id('material_piece_miter_corner'))id('material_piece_miter_corner').value=miter.corner||'';
    if(id('material_piece_miter_size'))id('material_piece_miter_size').value=miter.size||20;
    renderList();
  }
  function renderList(){
    var host=id('material_piece_list');if(!host)return;var term=String(value('material_piece_search','')).toLowerCase(),list=pieces();host.innerHTML='';
    list.filter(function(piece){return !term||piece.name.toLowerCase().indexOf(term)>=0;}).forEach(function(piece){var key=piece.materialKey||piece.pieceId,button=document.createElement('button');button.type='button';button.className='material-piece-button'+(key===selectedKey?' active':'');button.innerHTML='<span>'+piece.name+'</span><i class="material-chip" style="background:'+resolved(piece)+'"></i>';button.onclick=function(){if(window.Modular3DView)window.Modular3DView.selectPieceByKey(key);selectDetail({pieceId:piece.pieceId,materialKey:key,materialGroup:piece.group,materialName:piece.materialName,materialColor:piece.color,name:piece.name,ownerSpaceId:piece.ownerSpaceId});};host.appendChild(button);});
    var customCount=Object.keys(overrides).length,doors=list.filter(function(p){return p.category==='front';}).length,summary=id('material_summary');if(summary)summary.innerHTML='<span>'+list.length+' piezas</span><span>'+customCount+' personalizadas</span><span>'+doors+' frentes</span><span>'+(list.length?'Vista sincronizada':'Calculando...')+'</span>';
  }
  function refresh(){
  ...
  function renderSummary(){
    var host=id('material_build_summary'),validation=id('material_validation');if(!host)return;var list=pieces(),groupCounts={};list.forEach(function(p){var g=(overrides[p.materialKey]||{}).group||p.group;groupCounts[g]=(groupCounts[g]||0)+1;});
    host.innerHTML='<p><strong>Asignación:</strong> '+groups.map(function(g){return g+' '+(groupCounts[g]||0);}).join(' · ')+'</p><p><strong>Personalizadas:</strong> '+Object.keys(overrides).length+' pieza(s). Selecciona una pieza en azul para comprobar su acabado.</p>';
    if(validation){validation.textContent=list.length?'Materiales listos para construir. Los acabados individuales tienen prioridad sobre su grupo.':'Aún no hay piezas calculadas.';validation.className='message warn show';}
  }
  function applySelected(){
    if(!selectedKey||!selectedDetail){id('material_piece_info').textContent='Selecciona primero una pieza.';return;}
    var custom=id('material_piece_custom').value==='SI',group=id('material_piece_group').value,edge=id('material_piece_edge').value,edgeColor=id('material_piece_edge_color').value==='CUSTOM'?id('material_piece_edge_custom_color').value:'INHERIT',rotation=id('material_piece_rotation').value;
    if(custom||edge!=='INHERIT'||edgeColor!=='INHERIT'||pendingPieceTexture||rotation!=='INHERIT')overrides[selectedKey]={group:group,color:custom?id('material_piece_color').value:'',name:custom?(id('material_piece_name').value||'Material personalizado'):'',texture:pendingPieceTexture,rotation:rotation,edge:edge,edgeColor:edgeColor};else delete overrides[selectedKey];
    var deltaAncho=Number(value('material_piece_oversize_ancho',0))||0,deltaProf=Number(value('material_piece_oversize_prof',0))||0,deltaAlto=Number(value('material_piece_oversize_alto',0))||0;
    if(deltaAncho||deltaProf||deltaAlto)dimensionOverrides[selectedKey]={delta_ancho:deltaAncho,delta_prof:deltaProf,delta_alto:deltaAlto};else delete dimensionOverrides[selectedKey];
    var miterCorner=value('material_piece_miter_corner',''),miterSize=Number(value('material_piece_miter_size',20))||0;
    if(miterCorner&&miterSize>0)miterOverrides[selectedKey]={corner:miterCorner,size:miterSize};else delete miterOverrides[selectedKey];
    refresh();
  }
    function clearSelected(){if(selectedKey){delete overrides[selectedKey];delete dimensionOverrides[selectedKey];delete miterOverrides[selectedKey];}pendingPieceTexture='';if(id('material_piece_texture_included'))id('material_piece_texture_included').value='';if(id('material_piece_custom'))id('material_piece_custom').value='NO';if(id('material_piece_edge'))id('material_piece_edge').value='INHERIT';if(id('material_piece_edge_color'))id('material_piece_edge_color').value='INHERIT';if(id('material_piece_texture_url'))id('material_piece_texture_url').value='';if(id('material_piece_rotation'))id('material_piece_rotation').value='0';if(id('material_piece_oversize_ancho'))id('material_piece_oversize_ancho').value=0;if(id('material_piece_oversize_prof'))id('material_piece_oversize_prof').value=0;if(id('material_piece_oversize_alto'))id('material_piece_oversize_alto').value=0;if(id('material_piece_miter_corner'))id('material_piece_miter_corner').value='';if(id('material_piece_miter_size'))id('material_piece_miter_size').value=20;previewAt('material_piece_texture_preview','');refresh();}
  function bind(){
    if(bound)return;bound=true;
    var original=window.datosFormulario;
    window.datosFormulario=function(){var data=original();groups.forEach(function(group){data['material_'+group+'_color']=value('material_'+group+'_color','');data['material_'+group+'_nombre']=value('material_'+group+'_nombre','');data['material_'+group+'_custom']=checked('material_'+group+'_custom')?'SI':'NO';});data.material_unico=checked('material_unico')?'SI':'NO';data.material_global_color=value('material_global_color','#ffffff');data.material_global_nombre=value('material_global_nombre','Blanco');data.edge_mode=value('edge_mode','MIXED');data.material_overrides_json=JSON.stringify(overrides);data.material_textures_json=JSON.stringify(textures);data.material_texture_meta_json=JSON.stringify(textureMeta);data.dimension_overrides_json=JSON.stringify(dimensionOverrides);data.miter_overrides_json=JSON.stringify(miterOverrides);return data;};
    ...
    id('material_texture_file').addEventListener('change',function(){imageFileToData(this.files&&this.files[0],function(data){pendingTexture=data;if(id('material_texture_included'))id('material_texture_included').value='';previewTexture(data);},function(message){alert(message);});});
    if(id('material_texture_included'))id('material_texture_included').addEventListener('change',function(){if(!this.value)return;var tex=includedTexture(this.value);if(!tex)return;pendingTexture='INCLUDED:'+tex.id;id('material_texture_url').value='';id('material_texture_scale').value=tex.defaultScale;previewTexture(pendingTexture);});
